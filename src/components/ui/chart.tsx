...
>(({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload
    const key = `${labelKey || ("dataKey" in item ? item.dataKey : item.name) || "value"}`
    ...
    if (labelFormatter) {
      // Only pass string or number to labelFormatter
      let safeValue: string | number = "";
      if (typeof value === "string" || typeof value === "number") {
        safeValue = value;
      }
      const formatted = labelFormatter?.(safeValue);
      return typeof formatted === "string" || typeof formatted === "number" ? (
        <div className={cn("font-medium")}>{formatted}</div>
      ) : null
    }
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          const key = `${nameKey || item.name || ("dataKey" in item ? item.dataKey : "value")}`
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
...
>(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
  ...
  return (
    ...
    >
      {payload.map((item) => {
        const key = `${nameKey || ("dataKey" in item ? item.dataKey : item.value) || "value"}`
        const itemConfig = getPayloadConfigFromPayload(config, item, key)
